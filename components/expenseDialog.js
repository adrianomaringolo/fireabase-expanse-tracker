...
export default function ExpenseDialog(props) {
  ...
  const handleSubmit = async () => {
    ...
    const { id, date, locationName, address, items, amount, fileName, imageBucket } = formFields;

    try {
      if (isEdit) {
        if (fileName) {
          await replaceImage(file, imageBucket);
        }
        await updateReceipt(id, authUser.uid, date, locationName, address, items, amount, imageBucket);
      } else {
        await addReceipt(authUser.uid, date, locationName, address, items, amount, await uploadImage(file, authUser.uid));
